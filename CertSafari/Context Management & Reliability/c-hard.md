...
**어려운 이유** [원칙이 깨지는 예외, 덜 틀린 답 고르기] — "먼저 해결 시도 후 반복 요청 시 에스컬레이션"이라는 일반 패턴(D)이 강하게 끌리지만, 명시적 인간 요청에는 즉시 응해야 하는 예외다.
...
**어려운 이유** [덜 틀린 답 고르기] — 정책 내 해결(C)과 감정 무시 처리(D)가 결론이 같아, 좌절 인정 여부라는 미세한 차이로만 갈린다.
...
**어려운 이유** [덜 틀린 답 고르기] — 68번과 같은 구조지만 정답이 "직접 처리"가 아니라 "직접 처리 + 에스컬레이션 옵션 제시"여서, 68번식 규칙을 그대로 적용하면 틀린다.
...
**어려운 이유** [원칙이 깨지는 예외, 유사 현상 구분] — 요청이 사소하고 즉시 해결 가능해 C가 매우 매력적이지만, 명시적 인간 요청이 먼저 나왔다는 점에서 즉시 에스컬레이션이 정답인 경계 사례다.
