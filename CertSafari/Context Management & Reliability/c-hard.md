...
"명시적으로 사람 연결을 요청하면 무조건 즉시 에스컬레이션"이라는 단순 규칙은 이 문제 세트 전체를 관통하지 않습니다(2번 문제 참고: 같은 명시적 요청인데 답이 다름). 실제 기준은 **요청 뒤에 있는 작업이 판단·조사가 필요한 사안인가, 아니면 즉시 처리 가능한 단순 작업인가**입니다. 이 문제의 요금 분쟁은 계정 조사와 설명이 필요한 사안이므로, 지원 봇이 먼저 확인·해결을 시도(Attempt to resolve)하고, 그럼에도 고객이 반복 요청하거나 문제가 해결되지 않을 때 사람 상담원에게 이관하는 것이 올바른 흐름입니다.
...
1번 문제(요금 분쟁)와 비교하면 규칙이 더 분명해집니다: 요청 뒤 작업이 **조사·판단이 필요한 사안**이면 먼저 해결을 시도하지만, 이 문제처럼 **판단할 것 없이 즉시 처리 가능한 단순 작업**(비밀번호 재설정)이라면 AI가 대신 처리해보겠다고 나서는 것 자체가 사용자의 명시적 의사를 우회하는 행동이 됩니다. 즉 "작업이 쉬우니 AI가 먼저 처리해도 된다"는 논리가 성립하지 않는 지점이며, 오히려 작업이 단순할수록 굳이 AI가 처리를 시도할 이유가 없으므로 사용자의 요청을 그대로 존중해 즉시 에스컬레이션하는 것이 맞습니다.
