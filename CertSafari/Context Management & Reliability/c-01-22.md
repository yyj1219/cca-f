...
**has a customer raise**
고객이 제기하게 합니다

**a billing dispute**
청구 금액 분쟁을

**(invoice #4821, $89.00 overcharge)**
(청구서 #4821, $89.00 초과 청구)
...
**affecting their workspace**
그들의 작업 공간에 영향을 미치는

**(incident started 2:15 PM, status: unresolved).**
(사건 시작 시간 오후 2:15, 상태: 미해결).

**The agent's single running narrative summary**
에이전트의 단일 진행형 서술적 요약은

**blends both threads,**
두 스레드(이슈)를 혼합하며,

**and a later reply about "the amount"**
"금액"에 대한 이후의 답글이

**ambiguously could refer to**
모호하게 언급할 수 있게 됩니다
...
**in future multi-issue sessions?**
향후 다중 이슈 세션에서?

**OPTIONS**

**A) Maintain separate structured records**
별도의 구조화된 기록을 유지한다

**per issue:**
이슈별로:

**log invoice number and amount**
청구서 번호와 금액을 기록한다
...
**B) Summarize only the most recent issue,**
가장 최근의 이슈만 요약한다,

**such as the outage,**
서비스 중단과 같은,

**and treat earlier issues**
그리고 이전 이슈들을 처리한다

**like the billing dispute as resolved**
청구 분쟁과 같은 이슈를 해결된 것으로

**once a new topic arises.**
새로운 주제가 제기되면.

**C) Continue using one summary**
하나의 요약을 계속 사용한다

**but add issue-type labels:**
하지만 이슈 유형 라벨을 추가한다:

**"billing" for amounts,**
금액에는 "billing"을,

**"outage" for times and statuses,**
시간과 상태에는 "outage"를,

**to each mention of an amount or time.**
금액이나 시간이 언급될 때마다.

**D) Ask the customer to hold**
고객에게 보류하도록 요청한다

**the outage issue**
서비스 중단 이슈를

**until the billing dispute**
청구 분쟁이
...
**quickly locate the single function**
단일 함수를 빠르게 찾을

**that computes a discount rate**
할인율을 계산하는

**in a small module**
작은 모듈 내에서

**before making one edit.**
한 번의 수정을 하기 전에.
...
**A) Disable all direct tool use**
모든 직접적인 도구 사용을 비활성화한다
...
**must go through delegation.**
위임을 거치도록.

**B) Have the main agent search**
메인 에이전트가 검색하도록 한다

**and read the relevant file directly**
관련 파일을 직접 읽도록

**within its existing permitted**
기존 허용된 내에서

**and isolated context,**
그리고 격리된 컨텍스트,

**because the lookup is small**
조회가 작기 때문에

**and targeted enough**
충분히 타겟팅되어 있어서

**that subagent coordination overhead**
서브에이전트 조율 오버헤드가

**is not warranted.**
정당화되지 않는.

**C) Always spawn a subagent**
항상 서브에이전트를 생성한다

**regardless of task size,**
작업 크기에 관계없이

**since delegation is universally preferable**
위임이 보편적으로 선호되기 때문에

**for any exploration step.**
어떤 탐색 단계든.

**D) Spawn several subagents in parallel**
병렬로 여러 서브에이전트를 생성한다

**to search for the same function redundantly,**
동일한 함수를 중복하여 검색하기 위해

**to cross-check each other's answers.**
서로의 답을 교차 검증하기 위해.
...
**An architect is about to**
아키텍트가 막 하려고 한다

**delegate a codebase investigation**
코드베이스 조사를 위임하려고
...
**how to phrase the instruction.**
지시사항을 어떻게 작성할지.

**Which instruction best supports**
어떤 지시사항이 가장 잘 지원하는가

**getting a targeted,**
표적화되고,

**high-quality summary back**
고품질의 요약을 돌려받는 것을

**without still burdening**
여전히 부담을 주지 않으면서
...
**and let it infer the goal**
목표를 추론하게 한다

**from the main conversation's git status.**
메인 대화의 git 상태로부터.

**B) 'Report back**
'다시 보고하라

**the full, unsummarized contents**
요약되지 않은 전체 내용을
...
**C) 'Trace refund flow dependencies**
'환불 흐름 의존성을 추적하라

**across payments, ledger, and notifications,**
결제, 원장, 알림 전반에 걸쳐,

**and summarize what you find.'**
그리고 찾아낸 내용을 요약하라.'

**D) 'Look around the codebase**
'코드베이스를 둘러보라
...
**you happen to find interesting.'**
흥미롭다고 생각되는 것은 무엇이든.'
...
**that explores a large codebase**
대규모 코드베이스를 탐색하는

**over a run that may take hours**
몇 시간이 걸릴 수 있는 실행 동안
...
**They want the run to resume cleanly**
그들은 실행이 깨끗하게 재개되기를 원한다
...
**A) Rely on the conversation history**
대화 기록에 의존한다

**staying in context indefinitely,**
컨텍스트 내에 무기한 유지되는 것에,

**since the coordinator will always remember**
코디네이터가 항상 기억할 것이기 때문에

**every agent's progress unaided.**
도움 없이 모든 에이전트의 진행 상황을.

**B) Restart the entire exploration**
전체 탐색을 다시 시작한다

**from the beginning**
처음부터

**any time the process is interrupted,**
프로세스가 중단될 때마다,

**since partial progress cannot be recovered.**
부분적인 진행 상황은 복구될 수 없기 때문에.

**C) Have each agent keep its state**
각 에이전트가 상태를 유지하도록 한다

**only in its own memory**
자신의 메모리에만
...
**the coordinator can read later.**
나중에 코디네이터가 읽을 수 있는.

**D) Each agent exports its state**
각 에이전트가 자신의 상태를 내보낸다
...
**and the coordinator loads**
그리고 코디네이터가 로드한다

**a manifest of agent states**
에이전트 상태의 매니페스트를
...
**An insurance customer asks**
한 보험 고객이 묻는다

**whether their policy covers**
자신의 증권이 보장하는지 여부를

**a rental car**
렌터카 비용을

**while their electric vehicle's battery**
자신의 전기차 배터리가

**is being replaced**
교체되는 동안

**under a separate manufacturer recall.**
별도의 제조사 리콜에 따라.

**The policy documentation addresses**
보험 약관 문서에는 명시되어 있다

**rental reimbursement only**
렌터카 환급이 오직

**for collision and comprehensive claims,**
충돌 및 종합 보험 청구에 대해서만,
...
**A) Deny rental reimbursement,**
렌터카 환급을 거절한다,

**since the recall repair is not listed**
리콜 수리가 목록에 포함되어 있지 않기 때문에

**among the covered claim types**
보장되는 청구 유형 중에

**B) Approve rental reimbursement,**
렌터카 환급을 승인한다,

**since recall repairs are similar enough**
리콜 수리가 성격상 충분히 유사하기 때문에

**in nature to comprehensive collision claims**
종합 충돌 청구와

**C) Direct the customer to the manufacturer,**
고객을 제조사로 안내한다,

**since the recall is the underlying cause**
리콜이 근본적인 원인이기 때문에

**of the repair**
수리의

**D) Escalate the question,**
질문을 상급자/전문 부서로 이관(에스컬레이션)한다,

**since the policy documentation does not address**
보험 약관 문서가 다루지 않기 때문에

**this specific recall-related scenario**
이 특정 리콜 관련 시나리오를
