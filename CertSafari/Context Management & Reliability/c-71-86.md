...
C) Escalate the conversation to a human agent immediately, without first investigating the overage charge
...
고객의 이관(사람 연결) 요청에 응답하기 전에

**B) Offer to explain the overage charge in detail,**
초과 요금을 상세히 설명하겠다고 제안하고,

**attempt to resolve it,**
해결을 시도하며,

**and escalate to a human only**
오직 사람에게 이관합니다
...
문제가 해결되지 않은 상태로 남아있는 경우에만

**C) Escalate the conversation to a human agent immediately,**
대화를 즉시 사람 상담원에게 이관합니다,
...
C번: Escalate the conversation to a human agent immediately, without first investigating the overage charge

**정답 및 해설:**

**핵심 개념:** 

AI 가상 상담원(AI Support Agent) 설계 모범 사례에서 가장 중요한 원칙 중 하나는 고객의 '상담원 이관 명시적 요청(Explicit Human Escalation Request)'에 대한 즉각적인 이행입니다. 고객이 사람과의 대화를 명확히 요구한 경우, 자동화 시스템이 문제를 먼저 조사하거나 설득하려 하지 않고 지체 없이 사람 상담원에게 전달해야 합니다.

**문제 상황 분석:**

- 고객이 45달러 API 초과 요금 문제로 문의하며 "봇과 대화하기 싫으니 당장 사람 상담원을 연결하라"고 명시적으로 이관을 요구했습니다.
- 지원 에스컬레이션(Human Escalation) 프로토콜 및 Anthropic의 지원 가이드라인에 따른 최선의 AI 대응 방식을 선택해야 합니다.

**C번이 정답인 이유:**

고객이 사람 상담원 연결("connect me to a human agent right now")을 명시적으로 요구한 경우, AI 지원 시스템은 조사나 자체 해결 시도로 고객을 지체시키지 않고 요청을 즉시 받아들여 사람 상담원에게 이관(Escalate immediately)해야 합니다. 요구를 무시하고 조사를 진행하거나 설득하려 들면 고객의 불만과 좌절감이 더욱 극대화됩니다.

**오답 분석:**
- Option A (오답): 고객의 명시적인 사람 연결 요청을 무시하고 내부 조사를 먼저 진행하는 것은 고객 이관 요청 처리 원칙에 위배됩니다.
- Option B (오답): 봇과 대화하지 않겠다는 고객의 거부 의사를 무시하고 추가 설명을 시도하는 것은 불필요한 마찰을 유발하고 대화 만족도를 떨어뜨립니다.
- Option D (오답): 고객에게 자동화 시스템 거부 이유를 설명하도록 재차 요구하는 것은 불필요한 질문으로 이관 절차를 지연시킵니다.
